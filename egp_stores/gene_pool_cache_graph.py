...
class gene_pool_cache_graph():
    ...
    def add(self, gcs: list[aGC]) -> None:
        ...
        ancestry_edge_list = [(ngc.get(_VIDX, -1), self[ngc[_ANCESTOR_A]].get(_VIDX, -1), True) for ngc in gcs if ngc[_ANCESTOR_A]]
        ancestry_edge_list.extend([(ngc.get(_VIDX, -1), self[ngc[_ANCESTOR_B]].get(_VIDX, -1), True) for ngc in gcs if ngc[_ANCESTOR_B]])
        self._graph.add_edge_list(ancestry_edge_list, eprops=(self._graph.ep[_ANCESTRY],))
        structure_edge_list = [(ngc.get(_VIDX, -1), self[ngc[_GCA]].get(_VIDX, -1), True) for ngc in gcs if ngc[_GCA]]
        structure_edge_list.extend([(ngc.get(_VIDX, -1), self[ngc[_GCB]].get(_VIDX, -1), True) for ngc in gcs if ngc[_GCB]])
        self._graph.add_edge_list(structure_edge_list, eprops=(self._graph.ep[_STRUCTURE],))
    ...
    def remove(self, refs: list[int]) -> list[int]:
        ...
        while victims:
            ...
            descendant_edges = [e for e in self._graph.vertex(vgc_idx).in_edges() if self._graph.ep['ancestry'][e]]
            ancestor_a[_LOST_D] = ancestor_a.get(_LOST_D, 0) + vgc.get(_MISSING_A, 0) + vgc.get(_LOST_D, 0) + 1
            for descendant_edge in descendant_edges:
                ...
                if descendant[_ANCESTOR_A] == vgc_ref:
                    ...
                    descendant[_MISSING_A] = descendant.get(_MISSING_A, 0) + vgc.get(_MISSING_A, 0) + 1
                    self._graph.ep[_R_DIST_A][new_edge] = descendant.get(_MISSING_A, 0) + 1
                else:
                    descendant[_ANCESTOR_B] = ancestor_a_ref
                    descendant[_MISSING_B] = descendant.get(_MISSING_B, 0) + vgc.get(_MISSING_A, 0) + 1
                    self._graph.ep[_R_DIST_B][new_edge] = descendant.get(_MISSING_A, 0) + 1

            # If there is an ancestor B then it may have lost descendants (those that were missing links)
            if ancestor_b_ref:
                ancestor_b = self[ancestor_b_ref]
                ancestor_b[_LOST_D] = ancestor_b.get(_LOST_D, 0) + vgc.get(_MISSING_B, 0)
                assert vgc.get(_MISSING_B, 0) == 0, 'GC has no ancestor B but has missing links B!'

            # Remove structure edges
            sep = self._graph.ep[_STRUCTURE]
            for structure_edge in [e for e in self._graph.vertex(vgc_idx).out_edges() if self._graph.ep[_STRUCTURE][e]]:
